...
export default makeScene2D(function* (view) {
        ...
        <>
            <Rect
                ...
                shaders={{
                    ...
                    uniforms: {
                        mixColor: chroma(Solarized.base2).rgba().map(i => i / 256),
                        // mixColor: [0.5, 0, 0, 0],
            ...
            <Img x={-1100} ref={image} size={160} src={mario_run_1} smoothing={false} shadowColor={'black'} shadowBlur={5}/>
            <Layout ref={layout} layout top={() => new Vector2(0, 160 / 2)} width={160 * 13} wrap={'wrap'}>
                {
            ...
            <Circle ref={imageCircle} fill={Solarized.red} size={160} lineWidth={10} stroke={Solarized.base02} position={image().position} opacity={() => 1 - image().opacity()}/>
        </>
    );
    ...
    yield* all(
        ...
        delay(
            0.5,
            all(
                image().position(new Vector2(0, 0), 0.5, easeInExpo),
            )
        ),
        delay(
            0.85,
