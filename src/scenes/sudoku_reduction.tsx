...
import { Solarized, indicate, logPair, logValue, logPosition} from "../utilities";
import { createArc } from "../utilities_graph";
import { Sudoku, SudokuGraph, solution, clues, palette } from "../utilities_sudoku";
...
export default makeScene2D(function* (view) {
    ...
    yield* sequence(
        0.1,
        ...squares.map(square => square.opacity(1, 0.2))
    );
    yield* waitFor(1);
    ...
    yield* sequence(
        0.1,
        ...digits.map(digit => digit.opacity(1, 0.2))
    );
    yield* waitFor(1);

    // 6) Fade out the swatch and the digits
    yield* all(
        swatchRef().opacity(0, 0.5),
        digitsRef().opacity(0, 0.5)
    );
