...
export class Mobile extends Node {
    ...
    public constructor(props?: MobileProps) {
        ...
        this.add(
            <>
                <Rect ref={this.background} width={this.width} height={this.height} fill={Solarized.base01} radius={60}
                      zIndex={-2}/>
                <Rect ref={this.notchAround} width={this.background().width} height={this.background().height}
                      radius={this.background().radius}
                      lineWidth={20} stroke={Solarized.base03} scale={this.background().scale} zIndex={0}/>
                <Rect ref={this.notchTop} width={() => this.background().width() / 2}
                      height={this.notchAround().lineWidth}
                      fill={Solarized.base03} scale={this.background().scale} top={this.background().top}
                      opacity={this.background().opacity} radius={this.background().radius} zIndex={1}/>
                <Node ref={this.screenNode} cache zIndex={-1}>
                    <Rect position={this.background().position} scale={this.background().scale}
                          width={this.background().width} height={this.background().height}
                          radius={this.background().radius} fill={this.background().fill}/>
                    <Layout ref={this.passwordSection} layout scale={this.background().scale} direction={'column'}
                            gap={80}
                            alignItems={'center'} compositeOperation={'source-in'}>
                        <Layout layout direction={'column-reverse'} gap={10} alignItems={'center'}>
                            <Rect padding={8} ref={this.passwordField} lineDash={[71, 10, 10, 10]} radius={20}
                                  width={() => this.width() * (0.1 * props.pinLength)}
                                  height={60} lineWidth={10} gap={10} stroke={Solarized.cyan} justifyContent={'center'}>
                                {
                                    Array.from({length: props.pinLength}).map((_, i) =>
                                        <Txt ref={this.passwordTextPins[i]} text={`*`}
                        ...
                        <Layout wrap={'wrap'} width={() => this.background().width() * 0.8} justifyContent={'center'}
                                gap={() => this.background().width() * 0.7 / 16}>
                            {
                                [1, 2, 3, 4, 5, 6, 7, 8, 9, 0].map(i => {
                                    return <Circle ref={this.passwordCircles[i]}
                ...
                <Rect ref={this.lockscreen} position={this.background().position} scale={this.background().scale}
                      width={this.background().width} height={this.background().height}
                      radius={this.background().radius} fill={this.background().fill}
                      stroke={this.notchAround().stroke} lineWidth={this.notchAround().lineWidth}
                />
            </>
        );
    }
    ...
    public* spinPin() {
        yield* loop(() => this.passwordField().lineDashOffset(0).lineDashOffset(this.passwordField().lineDash().reduce((a, c) => a + c, 0), 3, linear));
    }
...
export default makeScene2D(function* (view) {
    ...
    yield* all(
        mobile().scale(0).scale(1, 1),
        mobile().opacity(0).opacity(1, 1),
    );
    ...
    yield loop(() => line().lineDashOffset(0).lineDashOffset(line().lineDash().reduce((a, c) => a + c, 0), 0.5, linear));
