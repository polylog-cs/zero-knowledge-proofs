...
export default makeScene2D(function* (view) {
    ...
    yield* sequence(
        0.3,
        ...graph.vertices.map(
            (vertex, i) => vertex.ref().fill(graph.palette[coloring[i]], 0.5)
        )
    );
    ...
    yield* all(
        ...
        graph.vertices[0].ref().position(
            graph.vertices[0].ref().position().add(new Vector2(-l/2, -l/2)), 
            1
        )
    )
    ...
    const sc = 4;
    yield* graph.edges[0].ref().lineWidth(
        graph.edges[0].ref().lineWidth() * sc, 
        1
    )
    yield* waitFor(1);
    yield* all(
